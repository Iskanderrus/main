...
def next_card():
    ...
    canvas.itemconfig(card_title, text="Armenian Letter", fill="black")
    canvas.itemconfig(card_word, text=current_card["Armenian Letter"], fill="black")
    canvas.itemconfig(card_background, image=card_front_img)
    flip_timer = window.after(3000, func=flip_card)


def flip_card():
    canvas.itemconfig(card_title, text=f"{current_card['Armenian Letter']}", fill="white")
